...
import { getStoreByCustomDomain } from "@/lib/firestore";
...
export default function CustomDomainCheckoutPage() {
  const router = useRouter();
  const params = useParams();

  // Example:
  // params.domain = "shop.ambica.com"
  ...
  useEffect(() => {
    ...
    async function loadCheckoutStore() {
      try {
        ...
        const storeData = await getStoreByCustomDomain(domain);
        ...
        console.error("Error loading custom domain checkout store:", error);
  ...
  const currentStoreMatchesCart = useMemo(() => {
    ...
    if (storeInfo?.slug) {
      return (
        ...
        String(storeInfo.slug) === String(domain)
      );
    }

    return true;
  }, [storeInfo, storeDoc, domain]);
    ...
    return (
      <>
        <Script
          ...
          strategy="afterInteractive"
        />
        <div className="p-6 text-red-500">Domain not found.</div>
      </>
    );
  }
        ...
        <div className="mx-auto max-w-6xl px-4 py-5">
          <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="rounded-[24px] bg-white p-4 shadow-sm md:p-6">
              ...
              <CheckoutForm
                storeSlug={storeDoc?.slug || domain}
                storeId={storeDoc.id}
